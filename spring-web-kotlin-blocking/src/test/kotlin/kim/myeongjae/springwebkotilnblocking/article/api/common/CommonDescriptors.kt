package kim.myeongjae.springwebkotilnblocking.article.api.common
...
class CommonDescriptors {
    ...
    class ConstrainedFields constructor(input: Class<*>?) {
        ...
        fun withPath(path: String): FieldDescriptor = fieldWithPath(path).attributes(
            key("constraints").value(constraintDescriptions.descriptionsForProperty(path).joinToString(". ")),
        )
    }
}
